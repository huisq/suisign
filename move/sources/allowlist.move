...
module suisign::allowlist;

use std::string::String;
use sui::dynamic_field as df;
use suisign::utils::is_prefix;
...
public fun add(allowlist: &mut Allowlist, cap: &Cap, account: address) {
    ...
    allowlist.signed.add(account, false);
}
...
public fun remove(allowlist: &mut Allowlist, cap: &Cap, account: address) {
    ...
    allowlist.signed.remove(account);
}
...
//doc sign functions:
public fun signdoc(id: vector<u8>, allowlist: &mut Allowlist, ctx: &TxContext) {
    assert!(approve_internal(ctx.sender(), id, allowlist), ENoAccess);
    assert!(table::borrow(&allowlist.signed, ctx.sender()) == false, EAlreadySigned);
    let signed_bool = table::borrow_mut<address, bool>(&mut allowlist.signed, ctx.sender());
    *signed_bool = true;
}

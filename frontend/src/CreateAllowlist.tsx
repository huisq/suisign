...
export function CreateAllowlist() {
  ...
  function createAllowlist(name: string) {
    if (name === '') {
      alert('Please enter a name for the doc sign');
      return;
    }
    ...
    signAndExecute(
      ...
      },
      {
        onSuccess: async (result) => {
          ...
          if (createdObjectId) {
            window.open(`${window.location.origin}/suisign/admin/doc/${createdObjectId}`, '_blank');
          }
        },
      },
    );
  ...
  const handleViewAll = () => {
    navigate(`/suisign/admin/all-suisigns`);
  };

  return (
    <Card>
      <h2 style={{ marginBottom: '1rem' }}>Admin View: SuiSigns</h2>
      <Flex direction="row" gap="2" justify="start">
        <input placeholder="Suisign Name" onChange={(e) => setName(e.target.value)} />
        <Button

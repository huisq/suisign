...
export function AllAllowlist() {
  ...
  return (
    <Card>
      <h2 style={{ marginBottom: '1rem' }}>Admin View: Owned SuiSigns</h2>
      <p style={{ marginBottom: '2rem' }}>
        These are all the SuiSigns that you have created. Click manage to edit the SuiSign and
        upload new files to the SuiSign.
      </p>
      {cardItems.map((item) => (
        <Card key={`${item.cap_id} - ${item.allowlist_id}`}>
          ...
          <Button
            onClick={() => {
              window.open(
                `${window.location.origin}/suisign/admin/doc/${item.allowlist_id}`,
                '_blank',
              );
            }}
          >
